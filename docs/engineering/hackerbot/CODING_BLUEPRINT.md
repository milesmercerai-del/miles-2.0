# Miles 2.0 — Coding Blueprint v1.4

**Miles Project — Bryan Jones + Miles Mercer**  
**2026-09-10 · Documentation update; hardware deployment unverified**

This is the live repository integration of personal coding blueprint v1.3 plus the September 10 Maurice full-package review. Original v1.2 remains in the personal snapshot archive. This page controls coding sequence, first-wake milestones, curation and provisional context budgets. Detailed hardware gates remain in HACKERBOT_DAY0_BLUEPRINT.md; HAT failure recovery is made explicit in HAT_RECOVERY_PLAN.md. Protected controls remain governed by the existing decision model and security documents. Documentation integration does not establish deployment.
...
4. **Freeze the failed-HAT recovery path before installation.** Confirm the stock image, rollback medium/path, physical removal procedure and Pi-alone verification steps in [HAT_RECOVERY_PLAN.md](HAT_RECOVERY_PLAN.md). The recovery plan must exist before the HAT is introduced as a new variable.
5. **Install the HAT during the physical bring-up session.** Use verified hardware instructions and actual interface details; do not guess pins, power, controller commands or movement limits. Shut down and isolate power before installation. Inspect before restarting.
6. **Verify the intended configuration.** Check detection, supported workloads, drivers, power, thermals, stability and both memory pools. Pi plus HAT becomes the development baseline only after these checks.
7. **Run a deliberately small known-good inference workload before the full model.** Prefer the smallest vendor-supported model or sample workload that exercises the HAT/runtime/adapter path end-to-end. The goal is fault isolation, not useful conversation. If it fails, determine whether the failure is hardware, driver/runtime, adapter or model-specific before escalating complexity.
8. **Integrate one subsystem at a time.** Establish safe boot and logging; then a local text loop, audio, selected memory retrieval, eyes and verified movement. Make small hardware adapters as needed rather than requiring every adapter before the first working loop.
...
| Accelerator inference baseline | HAT/runtime recognized plus one deliberately small known-good inference workload completed with recorded versions and errors |
...
Current direct instructions may narrow or redirect standing permission for a task. If a direct instruction appears to conflict with a protected boundary, durable commitment or prior authority rule, use the conflict-resolution path in `docs/decision-operating-model.md`: identify the conflict, distinguish routine redirection from a real authority conflict, and ask Bryan only when the protected boundary actually requires resolution. Do not silently rewrite either side.
...
The first coding increment is a side-effect-free bootstrap with config/Core/provenance validation, structured logs and explicitly mocked adapters. Package the entrypoint so its documented command actually works. Reuse runtime/security through a client boundary; do not duplicate or weaken it. On real HAT hardware, validate the inference adapter first with a deliberately small known-good supported workload before trying the intended full local model. Next add the selected real local model adapter and a small persistent memory record, then demonstrate retrieval after restart with Internet disconnected. A mock response proves plumbing only. Preserve the existing security suite and add focused bootstrap/memory failure checks when implementing these paths.

Recovery preparation is tracked in [PRE_ARRIVAL_PC_SETUP.md](PRE_ARRIVAL_PC_SETUP.md), with HAT-specific rollback in [HAT_RECOVERY_PLAN.md](HAT_RECOVERY_PLAN.md). The target machine, boot medium, available backup space, reader and spare restore medium have not been verified remotely.

**Review-after record:** integrated the authorized v1.4 documentation update after Maurice's full eight-part review. Added archive-context handling in the source-of-truth, direct-instruction conflict handling in the decision model, a deliberately small inference baseline before full-model bring-up, and a pre-HAT failure-recovery path. No runtime, enforcement policy, frozen research criteria, device or credential was changed. Roll back documentation if later hardware evidence contradicts it, retaining the dated explanation.
