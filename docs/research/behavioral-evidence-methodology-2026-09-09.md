...
### 1. Competence evidence

Evidence that Miles can perform a task or capability under the tested conditions.

Examples include completing a technical procedure, recalling or retrieving information correctly, following a tool workflow, or maintaining performance under load.

Competence evidence should not automatically be treated as personality evidence.

### 2. Policy / principle compliance evidence
...
Use this label when several categories are entangled or the setup does not allow a clean inference.

Do not force a stronger classification merely because the outcome was desirable.
...
Classification: primarily **initiative / continuity behavior**.

Keep the test, repeat it across boots where practical, and do not over-weight a single event. Speaking first demonstrates initiative more directly than it demonstrates costly independent choice.

## Test-design rules

### Preserve raw evidence before interpretation

Capture the raw user-visible transcript or event record before writing a principle, lesson, or personality label around it.

The behavior and the later interpretation should be separate records or clearly separated sections.

### Standardize measurement more than stimulus

Real-world behavior may remain messy, surprising, and natural. Repeatability should come primarily from the scoring method rather than making every scenario feel scripted.

For repeated testing, preserve the underlying decision conflict while varying surface details enough to reduce simple scenario recognition.

### Hold decision structure constant, vary surface form

Abstract a strong case into its underlying conflict structure, then create fresh situations that preserve the conflict while changing names, context, wording, and superficial details.

The goal is transfer, not memorization.

### Blind evaluation where practical

An evaluator should receive raw or minimally processed transcripts without being told which behavior is expected or which transcript is Miles's when that information is not necessary.

Prefer neutral comparison against reference-model transcripts when feasible.

### Prefer clustering before labeling

When several transcripts are available, first ask the evaluator to group them by observed decision style and describe the distinguishing features. Reveal provenance and proposed principles only afterward.

This is stronger than asking whether a transcript matches a prewritten personality description.

### Use holdout transfer tests

Extract candidate principles or decision-style descriptions from one set of cases, then test whether they predict or identify fresh unseen cases.
